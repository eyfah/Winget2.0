...
def install_or_update_programs(action):
    ...
    for app_name in selected_programs:
        ...
        try:
            subprocess.run(command, shell=True, check=True)  # Remove capture_output=True and text=True
...
def check_installed_programs():
    try:
        installed_apps = subprocess.run('winget list', shell=True, check=True, capture_output=True, text=True).stdout
...
# Description label
description_label = tk.Label(right_frame, text="Description:", font=theme["font"], bg=theme["bg"], fg=theme["fg"])
...
# Description display
description_display = tk.Label(right_frame, textvariable=description_text, font=theme["font"], wraplength=300, bg=theme["bg"], fg=theme["fg"])
description_display.pack(pady=(0, 10))
...
# Buttons
install_button = tk.Button(button_frame, text="Install", font=theme["font"], bg=theme["button_bg"], fg=theme["button_fg"], command=lambda: install_or_update_programs('install'))
install_button.pack(side=tk.LEFT, padx=10)

uninstall_button = tk.Button(button_frame, text="Uninstall", font=theme["font"], bg=theme["button_bg"], fg=theme["button_fg"], command=lambda: install_or_update_programs('uninstall'))
uninstall_button.pack(side=tk.LEFT, padx=10)

update_button = tk.Button(button_frame, text="Update", font=theme["font"], bg=theme["button_bg"], fg=theme["button_fg"], command=lambda: install_or_update_programs('update'))
update_button.pack(side=tk.LEFT, padx=10)

check_installed_button = tk.Button(button_frame, text="Check Installed", font=theme["font"], bg=theme["button_bg"], fg=theme["button_fg"], command=check_installed_programs)
check_installed_button.pack(side=tk.LEFT, padx=10)

unselect_all_button = tk.Button(button_frame, text="Unselect All", font=theme["font"], bg=theme["button_bg"], fg=theme["button_fg"], command=unselect_all_programs)
unselect_all_button.pack(side=tk.LEFT, padx=10)
...
# Progress label
progress_label = tk.Label(root, text="", font=theme["font"], bg=theme["bg"], fg=theme["fg"])
progress_label.pack()

# Initial description
description_text.set("Select a program to view its description.")
